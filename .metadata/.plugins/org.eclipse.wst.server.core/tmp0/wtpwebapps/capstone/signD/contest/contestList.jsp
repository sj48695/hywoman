...
<div class="bgi8">
   <section class="jumbotron text-center img-cover">
      <div class="container py-4">
         ...
         <p>
            <a href="contestList.jsp" class="btn btn-light btn-lg my-2">진행중</a>
...
<div class="container">
	<div class="row">
		...
		<div class="col-lg-4">
			...
			<hr class="my-3">
			<div class="articleMeta py-2" onclick="location.href='requestDetail.jsp?requestcode=<%=rq.getRequestcode()%>'">
   ...
   <div class="border-bottom p-2 row justify-content-between" style="line-height: 49px;">
      ...
      <div class="dropdown">
         ...
         <div class="dropdown-menu dropdown-menu-right">
            <a class="dropdown-item" href="contestList.jsp">전체</a>
            <a class="dropdown-item" href="contestList.jsp?category=logo">로고</a> 
            <a class="dropdown-item" href="contestList.jsp?category=package">패키지</a>      
            <a class="dropdown-item" href="contestList.jsp?category=web">웹</a>
            <a class="dropdown-item" href="contestList.jsp?category=other">기타</a>
         </div>
      </div>
   </div>
   <%
         ...
         <%
            if (count > 0) {
               ...
               if (endPage > pageCount)
                  endPage = pageCount;//2
         %>
         <a href="contestList.jsp?pageNum=<%=startPage - 10%>&keyword=<%=keyword %>"
            class="w3-button w3-hover-black<%if (startPage > 10) {} else {%> disabled<%}%>"><<</a>
         <%
               for (int i = startPage; i <= endPage; i++) {
                  if(!category.equals("")){%>
                  <a href="contestList.jsp?category=<%=rq.getCategory()%>&pageNum=<%=i%>&keyword=<%=keyword %>"
                  class="w3-button <%if (currentPage == i) {%>w3-black<%} else {%>w3-hover-black<%}%>"><%=i%></a>
              <%}else{ %>
                 <a href="contestList.jsp?pageNum=<%=i%>&keyword=<%=keyword %>"
                  class="w3-button <%if (currentPage == i) {%>w3-black<%} else {%>w3-hover-black<%}%>"><%=i%></a>
             <%}
            }
         %>
