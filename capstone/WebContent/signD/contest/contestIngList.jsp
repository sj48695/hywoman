...
<div class="bgi8">
   <section class="jumbotron text-center img-cover">
      <div class="container py-4">
         <h1 class="jumbotron-heading jumbotitle"><strong>랭킹 - 진행 중인 공모전 보기</strong></h1>
         <p class="lead"><strong>진행중인 공모전의 정보를 확인하세요!</strong></p>
         <p>
            <a href="contestIngList.jsp" class="btn btn-light btn-lg my-2">진행중</a>
            <a href="contestEndList.jsp" class="btn btn-outline-light btn-lg my-2">종료된</a>
         </p>
...
<div class="container">
	<div class="row">
		...
		<div class="col-lg-4">
			...
			<hr class="my-3">
			<div class="articleMeta py-2" onclick="location.href='detail.jsp?requestcode=<%=rq.getRequestcode()%>'">
...
<div class="container nationbottom">
   <div class="border-bottom p-2 row justify-content-between" style="line-height: 49px;">
      ...
      <div class="dropdown">
         ...
         <div class="dropdown-menu dropdown-menu-right">
            <a class="dropdown-item" href="contestIngList.jsp">전체</a>
            <a class="dropdown-item" href="contestIngList.jsp?category=logo">로고</a> 
            <a class="dropdown-item" href="contestIngList.jsp?category=package">패키지</a>      
            <a class="dropdown-item" href="contestIngList.jsp?category=web">웹</a>
            <a class="dropdown-item" href="contestIngList.jsp?category=other">기타</a>
         </div>
      </div>
   </div>
   <%
   ...
   <div style="position: relative; overflow: hidden;">
      ...
      <div class="new_list_item m-2 media row justify-content-between bgcolor<%=i%>">
         ...
         <div class="new_list_right col-sm-4 align-self-center">
            ...
            <div class="new_list_day"><strong>남은기간 : </strong> <span>D-<%=Dday%></span>(~<%=sdf.format(rq.getEnd_date())%>)</div>
         ...
         <%
            if (count > 0) {
               ...
               if (endPage > pageCount)
                  endPage = pageCount;//2
         %>
         <a href="contestIngList.jsp?pageNum=<%=startPage - 10%>&keyword=<%=keyword %>"
            class="w3-button w3-hover-black<%if (startPage > 10) {} else {%> disabled<%}%>"><<</a>
         <%
               for (int i = startPage; i <= endPage; i++) {
                  if(!category.equals("")){%>
                  <a href="contestIngList.jsp?category=<%=rq.getCategory()%>&pageNum=<%=i%>&keyword=<%=keyword %>"
                  class="w3-button <%if (currentPage == i) {%>w3-black<%} else {%>w3-hover-black<%}%>"><%=i%></a>
              <%}else{ %>
                 <a href="contestIngList.jsp?pageNum=<%=i%>&keyword=<%=keyword %>"
                  class="w3-button <%if (currentPage == i) {%>w3-black<%} else {%>w3-hover-black<%}%>"><%=i%></a>
             <%}
            }
         %>
