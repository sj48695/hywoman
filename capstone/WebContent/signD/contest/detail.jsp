            ...
            <td class="row align-items-end">
               <% if (rq.getId().equals(id) && Dday.equals("0")) { %>
                <button type="button" class="btn btnOr"
                	onclick="deleteCheck('/capstone/signD/contest/delPro.jsp?requestcode=<%=rq.getRequestcode()%>')">삭제하기</button> 
         ...
         <ul class="nav col-md-12 rq_nav" role="tablist">
            <li class="nav-item col-md-6">
               <a class="nav-link" data-toggle="tab" href="#participation"
                  role="tab" aria-controls="participation" aria-selected="true">참여작보기</a>
            </li>
            <li class="nav-item col-md-6">
               <a class="nav-link active" data-toggle="tab" href="#briefing" 
                   role="tab" aria-controls="briefing" aria-selected="false">브리핑보기</a>
            </li>
         </ul>
      </div>
   </nav> 
   <div class="tab-content" id="pills-tabContent">
      <div class="tab-pane fade show" id="participation" role="tabpanel">
         <jsp:include page="participation.jsp" flush="false">
            <jsp:param value="<%=rqcode%>" name="requestcode"/>
            <jsp:param value="<%=rq.getId()%>" name="requesterId"/>
         </jsp:include>
      </div>
      <div class="tab-pane fade show active" id="briefing" role="tabpanel">
         <jsp:include page="briefing.jsp" flush="false">
            <jsp:param value="<%=rqcode%>" name="requestcode"/>
         </jsp:include>
      </div>
   </div>
                       ...
                       %>
                          ...
                          %>
                                <button type="button" class="close" name="replydelete"
                                   onclick="location.href='delPro.jsp?replycode=<%=reply.getReplycode()%>&requestcode=<%=reply.getRequestcode()%>'">
			...
			<%
				if (recount > 0) {
					...
					if (endPage > pageCount)
						endPage = pageCount;//2
			%>
				<a href="detail.jsp?pageNum=<%=startPage - 10%>&requestcode=<%=rqcode%>"
					class="w3-button w3-hover-black<%if (startPage > 10) {} else {%> disabled<%}%>"><<</a>
				<%
					for (int i = startPage; i <= endPage; i++) {
				%>
				<a href="detail.jsp?pageNum=<%=i%>&requestcode=<%=rqcode%>"
