		...
		<div class="modal-body p-0">
			<div class="media row">
				...
				<div class="col-md-4 pl-3" id="content<%=ppcode %>" style="background-color: #e9ecef;">
					...
					<h4 class="py-4 m-0"><a href="/capstone/signD/contest/detail.jsp?requestcode=<%=rqcode%>"><%=pp.getTitle() %></a></h4>
					...
					%> 
						<button onclick="location.href='/capstone/signD/designer/participation/updateForm.jsp?ppcode=<%=pp.getPpcode()%>&rqcode=<%=rqcode%>'"
							class="btn btnOr offset-1 col-md-5">수정하기</button> 
						<button onclick="deleteCheck('/capstone/signD/designer/participation/delPro.jsp?ppcode=<%=pp.getPpcode()%>&rqcode=<%=rqcode%>')"
							class="btn col-md-5" style="background-color: darkgray; color: white;">삭제하기</button>
					<%
							} else {//마감후 포트폴리오
					%> 
						<button onclick="portfolioInsert('/capstone/signD/designer/portfolio/registerForm.jsp?ppcode=<%=pp.getPpcode()%>&rqcode=<%=rqcode%>&category=<%=rq.getCategory()%>')"
...
<script>
//채택하기/취소하기(detail.jsp->participation.jsp)
