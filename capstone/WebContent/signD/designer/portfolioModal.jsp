			...
			<div class="media row">
				...
				%>
					...
					<div class="carousel-inner" >
						<div class="carousel-item active">
							<img class="d-block w-100" src="/capstone/fileSave/portfolioFile/<%if(rqcode!=0){%><%=rq.getCategory() %>/<%} %><%=filenames[filenames.length-1]%>">
						</div>
				<%
					for (int j = filenames.length - 1; j > 0; j--) {
				%>
						<div class="carousel-item middle">
							<img class="d-block w-100" src="/capstone/fileSave/portfolioFile/<%if(rqcode!=0){%><%=rq.getCategory() %>/<%} %><%=filenames[j - 1]%>">
						</div>
				<%
					}
				%>
				...
				<div class="col-md-4 pl-3" id="content<%=pfcode%>" style="background-color: #e9ecef;">
					<h4 class="py-4 m-0">
						...
						%>
						<a href="/capstone/signD/contest/requestDetail.jsp?requestcode=<%=rqcode%>"><%=pf.getTitle()%></a>
						<%	} %>
					...
					<div class="pr-4 p-3" aria-pressed="false">
						...
						%>
						<button onclick="location.href='/capstone/signD/designer/portfolioUpdateForm.jsp?pfcode=<%=pf.getPortfoliocode()%>&rqcode=<%=rqcode%>'"
							class="btn btnOr offset-1 col-md-5">수정하기</button> 
						<button onclick="deleteCheck('/capstone/signD/designer/portfolioDeletePro.jsp?pfcode=<%=pf.getPortfoliocode()%>&rqcode=<%=rqcode%>')"
