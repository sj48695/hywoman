...
<div class="modal-dialog modal-dialog-centered modal-lg" role="document" style="max-width:1000px">
	<div class="modal-content">
		<div class="modal-body p-0">
			<div class="media row">
				<div id="carouselFade<%=answer.getAnswercode() %>" class="carousel slide pr-0 carousel-fade col-md-8 align-self-center" data-ride="carousel">
					<ol class="carousel-indicators">
				<%
					if(filenames.length>1){
						for (int j = 0; j < filenames.length ; j++) {
				%>
						<li data-target="#carouselFade<%=answer.getAnswercode()%>" data-slide-to="<%=j %>" <%if(j==0){%>class="active"<%} %>></li>
				<%
						}
					}
				%>
					...
					<div class="carousel-inner">
						<div class="carousel-item active">
							<img class="d-block w-100" src="/capstone/fileSave/answerFile/<%=rq.getCategory()%>/<%=filenames[filenames.length-1]%>">
						</div>
				<%
					for (int j = filenames.length - 1; j > 0; j--) {
				%>
						<div class="carousel-item">
							<img class="d-block w-100" src="/capstone/fileSave/answerFile/<%=rq.getCategory()%>/<%=filenames[j - 1]%>">
						</div>
				<%
					}
				%>
				...
				<%
					if (filenames.length > 1) {
				%>
					<a class="carousel-control-prev" href="#carouselFade<%=answer.getAnswercode()%>" role="button" data-slide="prev"> 
						<i class="fas fa-angle-left"></i>
						<span class="sr-only">Previous</span>
					</a> 
					<a class="carousel-control-next" href="#carouselFade<%=answer.getAnswercode()%>" role="button" data-slide="next"> 
				...
				<div class="col-md-4 pl-3" id="content<%=answer.getAnswercode()%>" style="background-color: #e9ecef;">
					...
					<h4 class="py-4 m-0"><a href="/capstone/signD/contest/requestDetail.jsp?requestcode=<%=rqcode%>"></a></h4>
					...
					%>
						<button onclick="location.href='/capstone/signD/otorequest/OTOAnswerUpdateForm.jsp?answercode=<%=answer.getAnswercode()%>&rqcode=<%=rqcode%>'"
							class="btn btn btnOr offset-1 col-md-5">수정하기</button> 
						<button onclick="deleteCheck('/capstone/signD/otorequest/OTOAnswerDeletePro.jsp?answercode=<%=answer.getAnswercode()%>&rqcode=<%=rqcode%>')"
